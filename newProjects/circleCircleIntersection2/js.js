...
function makeParticle(numParticles) {
  ...
  for(var i=0; i<numParticles; i++){
    var p = {
    x : random(p.size*2,canvasW-(p.size*2)),
    y : random(p.size*2,canvasH-(p.size*2)),
    xVel : random(-6,6),
    yVel : random(-6,6),
    hue : random(360),
    size : random(30,50),
    ...
    c.globalCompositeOperation = "sorce-over";
...
{
setTimeout(function(){particleTest = false;},500);
}
    myFunction();
    return false ;
    preventDefault();
...
var particleTest = false;

    var testParticle = [];

    var cTestX = 300;
    var cTestY = 300;
    var cTestXvel = random(-6,6);
    var cTestYvel = random(-6,6);
    var cTestSize = 100;
    var cTestColor = rgb(random(0,255),random(0,255),random(0,255));
...
if (particleTest) {
    ...
    c.strokeStyle = cTestColor;
    c.strokeCircle(cTestX,cTestY,cTestSize);


    cTestX = cTestX + cTestXvel;
    cTestY = cTestY + cTestYvel;
    ...
    if (cTestX >= canvas.width - (cTestSize) ){
      cTestX = canvas.width - (cTestSize);
      cTestXvel*=-1;
    } // close if 
    ...
    if (cTestY >= canvas.height - (cTestSize) ){
      cTestY = canvas.height - (cTestSize);
      cTestYvel*=-1;
    } // close if
